...
<?php
// 1) Incluye bien la conexión:
include('./constant/connect.php');
// 2) Inicializa el contador
$no = 0;
$user = $_SESSION['userId'];
$sql = "SELECT uno, orderDate, clientName, clientContact, paymentStatus, id
        FROM orders
                        ...
                        <tbody>
                            <?php foreach ($result as $row): $no++; ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        if ($row['paymentStatus'] == 1)      echo "<label class='label label-success'><h4>Pago Completo</h4></label>";
                                        elseif ($row['paymentStatus'] == 2)  echo "<label class='label label-danger'><h4>Pago Parcial</h4></label>";
                                        else                                 echo "<label class='label label-warning'><h4>Pago Pendiente</h4></label>";
                                        ?>
                                    </td>
                                    <td>
                                        <a href="editorder.php?id=<?= $row['id'] ?>">
                                            <button class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i></button>
                                        </a>
                                        <a href="php_action/removeOrder.php?id=<?= $row['id'] ?>" onclick="return confirm('Deseas eliminar este registro?')">
                                            <button class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>
                                        </a>
                                        <a href="invoiceprint.php?id=<?= $row['id'] ?>">
