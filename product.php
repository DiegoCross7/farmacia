...
<?php include('./constant/connect'); // Mantenemos para las consultas adicionales

// Reemplazamos la consulta SQL por la llamada a la API
// Usando cURL para mayor compatibilidad
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "http://127.0.0.1:3001/api/productos"); // Usamos IP en lugar de localhost
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5); // Timeout más corto
$response = curl_exec($ch);
$error = curl_error($ch);
curl_close($ch);

if (!empty($error)) {
    // Si hay error, mostrar mensaje pero seguir
    echo "<div class='alert alert-danger'>Error: $error. Usando base de datos directa.</div>";

    // Fallback a la consulta directa si falla la API
    ...
    // Usamos la respuesta de la API
    $result = json_decode($response, true);

    // Si la respuesta no se puede decodificar, usamos la consulta SQL
    if ($result === null) {
        echo "<div class='alert alert-danger'>Error al decodificar respuesta. Usando base de datos directa.</div>";
        $sql = "SELECT product_id, product_name,product_image,rate,quantity,brand_id,expdate,categories_id,active,status FROM product WHERE status = 1";
        $result = $connect->query($sql);
    }
}
...
<div class="page-wrapper">
    ...
    <div class="container-fluid">
        ...
        <style>
            ...
            /* Mejora en imágenes de productos */
            table.table img {
